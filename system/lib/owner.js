...
let premium = [];
try {
premium = JSON.parse(fs.readFileSync("./system/database/owner.json", "utf8"));
if (!Array.isArray(premium)) throw new Error("File owner.json harus berupa array!");
} catch (err) {
console.error("⚠️ Gagal membaca owner.json, menggunakan array kosong.");
premium = [];
}
...
try {
fs.writeFileSync("./system/database/owner.json", JSON.stringify(_dir, null, 2));
console.log("📝 Database premium diperbarui!");
} catch (error) {
console.error("❌ Gagal menyimpan owner.json:", error);
return false;
}
return true;
};;
